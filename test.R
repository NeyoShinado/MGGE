...
for(lambda1 in 10^seq(-2, 6, 4)){
  for(lambda2 in 10^seq(-2, 6, 4)){
    tryCatch({
      for(i in 2){
        #try({
        s = Sys.time()
        ...
        res <- var_update(lg_X, K, npc, lambda1=lambda1, lambda2=lambda2, 
                          iteration=1, clust_iteration=50, imp_iteration=1000, 
                          res_save=FALSE)
        clust = res$cluster
        nmi = NMI(clust, gt)
        ari = ARI(clust, gt)
        ...
        saveRDS(res, paste0("result/lambda/", output, "_", lambda1, "_", lambda2, '.rds'))
        #})
      }
